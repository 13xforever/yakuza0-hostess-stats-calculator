...
*/
                ...
                using (var writer = new StreamWriter(output, new UTF8Encoding(false)))
                {
                    ...
                    foreach (var os in oc.Value.OrderBy(i => i.cost))
                    {
                        ...
                        var o = os.outfit;
                        writer.WriteLine(
                            $"{hostess.Name};{ol.Key};{s.s};{s.b};{s.c};{s.f};{os.cost};{o["DRESS"].Name};{o["HAIR"].Name};{o["HAIRACC"].Name};{o["GLASSES"].Name};{o["EARRING"].Name};{o["NECKLACE"].Name};{o["NAIL"].Name};{o["RING"].Name};{o["WATCH"].Name};{o["BRACELET"].Name}");
                    }
                }
            }
